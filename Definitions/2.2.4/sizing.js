...
for(let i = 0; i < SCALES.MAX_WIDTH_SCALE.length; i++) {
  maxWidth.push(
    {
      label:         `max-w-${SCALES.MAX_WIDTH_SCALE[i].name}`,
      detail:        `max-width: ${SCALES.MAX_WIDTH_SCALE[i].value};`,
      documentation: `Set the maximum element width to ${SCALES.MAX_WIDTH_SCALE[i].value}.`
    }
  )
}
